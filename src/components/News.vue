<template>
  <section class="news">
    <div class="container">
      ...
      <div class="row">
        <div v-for="myNew in news" :key="myNew.id" :class="{'col-md-12': myNew.id === 1 , 'col-md-6' : myNew.id !== 1}">
          <div class="news__box">
            <img :src="myNew.img" />
            <h3>{{myNew.title}}</h3>
            <p>{{myNew.text}}</p>
            <router-link :to="{path: `/noticia/${myNew.id}`, params: { id: myNew.id }}" ><span>Leia mais</span></router-link>
          </div>
        </div>
        <div class="col-md-12">
